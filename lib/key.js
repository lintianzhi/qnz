...
function getMultipart(upToken, key, body, extra) {
  ...
  form.buffer('file', body, key||'?', mimeType);
...
Key.prototype.pfop = function(fops, notifyUrl, onret) {
  var uri = this.attr('apiHost')+'/pfop/';
  var body = util.format("bucket=%s&key=%s&fops=%s&notifyURL=%s", this.attr('bucketName'), this.attr('keyName'), fops, notifyUrl);
